...
export const staticTransition = {
  duration: 0.25,
  ease: cubicBezier(0.36, 0.67, 0.22, 1),
};
        ...
        >
          {({ state }) => (
            ...
            >
              ...
              >
                {notch && side === "bottom" && (
                  <div className="notch sticky top-0 mx-auto mt-1.5 h-1 w-10 shrink-0 touch-pan-y rounded-full bg-fg/20" />
                )}
                {children as React.ReactNode}
                {notch && side === "top" && (
                  <div className="notch sticky bottom-0 mx-auto mb-2.5 h-1 w-10 shrink-0 touch-pan-y rounded-full bg-fg/20" />
                )}
              </Dialog>
            </DrawerRoot>
          )}
...
const DrawerBody = ({
  ...
  hasNoScroll = false,
  ...props
}: React.HTMLAttributes<HTMLDivElement> & { hasNoScroll?: boolean }) => (
  <div
    slot="body"
    className={twMerge(
      `isolate flex max-h-[calc(var(--visual-viewport-height)-var(--visual-viewport-vertical-padding))] flex-col ${hasNoScroll ? 'overflow-unset' : 'overflow-auto'} px-4 py-1 will-change-scroll`,
      className
    )}
    {...props}
  />
